...
src/server/routes/   - 27 Express route files (projects, containers, LLM, branch review, skills, chat, scheduler, slack, etc.)
src/server/models/   - 6 data models: Project, Prompt, Plan, GlobalRule, History, ChatMessage (SQLite-backed via sqliteStore.js)
src/client/src/pages/      - 11 pages: IDE, Dashboard, Onboarding, BranchReview, Skills, Profile, GlobalRules, CreateProject, EditProject, QuickPrompt, ProjectDetail
...
data/                - Runtime data: app.sqlite (SQLite primary store), session history, chat logs, job state
docker/              - Dockerfile.dev for building project containers
scripts/             - Utility scripts (PTY permissions fix)
...
7. **Scheduled Tasks** -- Cron-like tasks with LLM-assisted configuration, running inside containers.
8. **Chat System** -- Per-project chat with unread counts and notifications.
9. **Slack Integration** -- Bot integration via @slack/bolt.
10. **Agent Orchestration** -- Agent gateway, shell pool, auto-responder, and safety system for autonomous AI operations.
11. **Onboarding Wizard** -- 3-step setup: connect AI model, install Docker, create first project.
