/**
 * Client grouping and sort order for IDE projects.
 * Client names are compared with exact trimmed equality (not case-folded).
 */
...
export function normalizeSortOrder(value, fallback = 0) {
  const n = Number(value);
  return Number.isFinite(n) ? n : fallback;
}

export function nextSortOrder(projects) {
  if (!projects?.length) return 0;
  return Math.max(...projects.map((p) => normalizeSortOrder(p.sortOrder, -1))) + 1;
}

export function clientKey(project) {
  return normalizeClient(project?.client);
}

export function defaultSortOrder(project) {
...
export function normalizeOrganization(project) {
  if (!project) return project;
  project.client = normalizeClient(project.client);
  project.sortOrder = defaultSortOrder(project);
  return project;
}

/**
 * @param {Array<{ id: string, name?: string, client?: string, sortOrder?: number }>} projects
 * @returns {Array<{ client: string, sortOrder: number, projects: object[] }>}
 */
...
function reindexFromGroups(original, groups) {
  const orderById = new Map();
  groups.forEach((group) => {
    group.projects.forEach((project) => {
      orderById.set(project.id, orderById.size);
    });
  });
...
export function moveProjectInClientGroup(projects, projectId, direction) {
  const groups = groupProjectsByClient(projects);
  const group = groups.find((g) => g.projects.some((p) => p.id === projectId));
  if (!group) return { projects, moved: false };

  const idx = group.projects.findIndex((p) => p.id === projectId);
  const swapIdx = direction === 'up' ? idx - 1 : idx + 1;
  if (swapIdx < 0 || swapIdx >= group.projects.length) {
    return { projects, moved: false };
  }

  const items = [...group.projects];
  [items[idx], items[swapIdx]] = [items[swapIdx], items[idx]];
  group.projects = items;
  return { projects: reindexFromGroups(projects, groups), moved: true };
}

export function moveClientGroup(projects, client, direction) {
  const key = normalizeClient(client);
  const groups = groupProjectsByClient(projects);
  const idx = groups.findIndex((g) => g.client === key);
  if (idx < 0) return { projects, moved: false };

  const swapIdx = direction === 'up' ? idx - 1 : idx + 1;
  if (swapIdx < 0 || swapIdx >= groups.length) {
    return { projects, moved: false };
  }
...
export function sortProjects(projects) {
  return groupProjectsByClient(projects).flatMap((group) => group.projects);
}
